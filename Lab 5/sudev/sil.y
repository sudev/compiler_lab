...
general_stmt_seq:
		...
		;
general_stmt:
		IF general_expr THEN stmt_block ENDIF ';' 				{ $$ = create_node(T_IF, $2, NULL); $2->next = $4; if (!check_bool($2)) yyerror("Bad if condition!"); }
		| IF general_expr THEN stmt_block ELSE stmt_block ENDIF ';'		{ $$ = create_node(T_IF, $2, NULL); $2->next = $4; $4->next = $6; if (!check_bool($2)) yyerror("Bad if condition!"); }
		| WHILE general_expr DO stmt_block ENDWHILE ';'				{ $$ = create_node(T_WHILE, $2, NULL); $2->next = $4; if (!check_bool($2)) yyerror("Bad while condition!"); }
		| WRITE '(' general_expr ')' ';'					{ $$ = create_node(T_WRITE, $3, NULL); }
		| READ '(' variable_rw ')' ';'						{ $$ = create_node(T_READ, $3, NULL); }
		| variable_rw '=' general_expr ';'					{ $$ = create_node('=', $1, NULL); $1->next = $3; if (!((check_bool($1) && check_bool($3)) || (check_int($1) && check_int($3)))) yyerror("Assignment failed!"); }
		;
general_expr:
